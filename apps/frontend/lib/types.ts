...
/**
 * @see https://vercel.com/docs/rest-api/endpoints#get-a-project-domain
 */
export interface DomainResponse {
   ...
   * If `false` it will not be used as an alias on this project until
   * the challenge in `verification` is completed.
   */
  verified: boolean
  /**
   * A list of verification challenges, one of which must be completed to verify the domain for use on the project.
   * After the challenge is complete `POST /projects/:idOrName/domains/:domain/verify` to verify the domain.
   * Possible challenges: - If `verification.type = TXT` the `verification.domain` will be checked for a TXT record
...
export interface DomainConfigResponse {
   ...
   * - `CNAME` Domain has a CNAME pointing to Vercel.
   * - `A` Domain's A record is resolving to Vercel.
